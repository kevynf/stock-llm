...
<table>
  ...
  <tr>
    ...
    <td align="center"><strong>带来源信息的个股研究</strong></td>
  </tr>
</table>
...
实现细节单独记录在[架构文档](docs/architecture.zh-CN.md)中。
...
默认端点为 `https://api.deepseek.com`，模型为 `deepseek-chat`。建议在应用的“设置”页输入 API 密钥，由后端通过 `keyring` 写入系统密钥库。

请妥善保管密钥。未配置密钥时，StockLLM 仍会生成规则候选和证据，并明确标记“AI 待连接”。

DeepSeek 只参与候选比较和只读研究对话。模型不能修改自选股、研究记录或设置，也不能绕过候选池和证据引用校验。
...
行情与公司新闻来自 AkShare 和东方财富，日线来自 BaoStock，公告来自巨潮资讯。界面会在相应数据旁展示来源和有效日期。
...
欢迎提交 Issue 和 Pull Request。提交改动前请先阅读[贡献指南](CONTRIBUTING.zh-CN.md)。
